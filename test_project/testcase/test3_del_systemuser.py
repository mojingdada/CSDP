...
excel_search = readexcel.ExcelUtil('case.xlsx', 'del')
data_login = excel_login.dict_data()
data_del = excel_search.dict_data()
...
class test2_search_del_systemuser(unittest.TestCase):
    ...
    def test5_deluser(self):
        u''' 删除测试，删除搜索出的系统用户信息'''
        browser = self.browser
        self.more = browser.find_element_by_xpath("//div[contains(@ng-if,'operation.moreButtons.length')]").click()
        self.deluser = browser.find_element_by_xpath("//i[contains(@class,'fa fa-trash icon-btn-remove')]").click()
        self.success = browser.find_element_by_xpath("//a[contains(@class,'btn btn-xs btn-success')]").click()
